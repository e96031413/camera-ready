...
**Theme injection:** `scripts/pptx_render_diagrams.py` auto-injects theme-colored defaults:
```dot
...
Shares LaTeX compilation pipeline with `scripts/pptx_render_latex.py`. Auto-includes `tikz`, `amsmath`, `amssymb`, and common TikZ libraries (`arrows.meta`, `positioning`, `calc`, `shapes`, `fit`, `backgrounds`).
...
See `references/pptx-pptxgenjs-reference.md` → Shapes section for API.
...
Three helper functions (defined in `references/pptx-themes.md` script template):
