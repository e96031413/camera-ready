...
### Academic paper skills (LaTeX papers, literature reviews, citations)

**Bad evals:**
- "Is the paper well-written?" (subjective)
- "Rate the academic rigor 1-10" (scale)

**Good evals:**
- "Does every paragraph of 3+ sentences contain at least one citation?" (binary, countable)
- "Does the paper compile without errors via pdflatex + bibtex?" (binary, testable)
- "Are all BibTeX entries verified against DBLP/CrossRef/arXiv?" (binary, checkable)
- "Does the abstract contain: problem statement, approach, key findings, and significance — each in ≤2 sentences?" (binary, structural)
- "Is the paper free of AI-pattern phrases from the banned list (e.g., 'delve', 'it is worth noting', 'in recent years')?" (binary, greppable)
- "Does every figure/table have a caption AND at least one in-text reference?" (binary, structural)
...
```
EVAL 1: Citation density
Question: Does every body paragraph (≥3 sentences) contain at least one \cite{} command?
Pass: All body paragraphs have citations; only abstract/conclusion may omit
Fail: Any body paragraph of 3+ sentences has zero citations
```
