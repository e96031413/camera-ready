...
```bash
python scripts/pptx_render_latex.py formulas.json formulas/              # PNG @ 600 DPI (default)
python scripts/pptx_render_latex.py formulas.json formulas/ --dpi 900    # PNG @ custom DPI
python scripts/pptx_render_latex.py formulas.json formulas/ --format svg # SVG (requires ghostscript)
```
...
OMML entry (no file generated — scripts/pptx_inject_omml.py handles conversion at post-processing):
```json
{
...
The `addFormula()` helper in `references/pptx-themes.md` script template auto-routes OMML vs image:

- **OMML formulas**: writes `{{MATH:id}}` placeholder text → `scripts/pptx_inject_omml.py` replaces with native math post-generation
...
```bash
python scripts/pptx_inject_omml.py output.pptx formulas.json output.pptx
```
...
1. `scripts/pptx_render_latex.py` marks formulas with `render:"omml"` in manifest (no image generated)
2. PptxGenJS script writes `{{MATH:id}}` placeholder text via `addMathText()` helper
3. `scripts/pptx_inject_omml.py` post-processes the .pptx:
...
| Post-processing | scripts/pptx_inject_omml.py required | None |

## Error Recovery

If `scripts/pptx_render_latex.py` reports errors:
