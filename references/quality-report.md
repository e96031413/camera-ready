...
- Paper-Reviewer loop complete: Review issues are `DONE` (or `SKIP` with justification)
...
- BibTeX audit is healthy: `python scripts/bibtex_audit.py --project-dir <paper_dir> --window-years 3` (fix duplicates/missing years)

## Integrity checks (claim-level)
- Claim registry exists and is filled: `python scripts/claim_registry.py --project-dir <paper_dir> --mode broad --write-csv`
- Final integrity gate passes (Stage 4.5): `python scripts/integrity_gate.py --project-dir <paper_dir> --stage final`
...
## Delivery checks
- Page count within target (main text only): from this skill folder, run `python scripts/compile_paper.py --project-dir <paper_dir> --report-page-counts` (requires a bibliography-start label; default `ReferencesStart`) and use "Main text pages (exclude ref-start page)".
...
- Privacy scan passed: `python scripts/paper_privacy_scan.py --project-dir <paper_dir>` (exit 0)
