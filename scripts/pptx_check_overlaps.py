...
Usage:
  python pptx_check_overlaps.py input.pptx [--json report.json] [--min-overlap 0.01] [--min-gap 0.05]
...
def _is_full_dark_slide(slide_xml: bytes) -> bool:
    """Detect title/section/thank-you slides (full dark background)."""
...
def check_slide(elements: list[BBox], slide_num: int,
    ...
    # 1. Boundary check (skip for title/section/thank-you slides)
    ...
    # 3. Vertical gap check
    top_level = [e for e in check_elems
                 if e.group_id < 0 or e.has_fill]
