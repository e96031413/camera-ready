...
Usage:
  python pptx_render_latex.py formulas.json output_dir/                  # PNG @ 600 DPI (default)
  python pptx_render_latex.py formulas.json output_dir/ --dpi 900        # PNG @ custom DPI
  python pptx_render_latex.py formulas.json output_dir/ --format svg     # SVG (needs ghostscript)
...
def _has_ghostscript() -> bool:
    ...
    for cmd in ("gs", "gswin64c", "gswin32c"):
        try:
            subprocess.run([cmd, "--version"], capture_output=True, encoding="utf-8", errors="replace", timeout=5)
            return True
...
def _compile_to_cropped_pdf(formula: dict, tmpdir: str) -> tuple[str | None, str | None]:
    ...
    r2 = subprocess.run(
        ...
        capture_output=True, encoding="utf-8", errors="replace", text=True, timeout=15,
    )
...
def _render_svg(fid: str, cropped_pdf: str, outdir: Path) -> dict:
    ...
    svg_text = final.read_text(encoding="utf-8", errors="replace")[:2000]
    w_pt, h_pt = 0.0, 0.0
...
def main():
    ...
    with open(args.formulas, encoding="utf-8") as f:
